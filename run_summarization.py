...
"""This is the top-level file to train, evaluate or test your summarization models"""
...
def convert_to_coverage_model():
    ...
    tf.logging.info("converting non-coverage models to coverage models..")

    # initialize an entire coverage models from scratch
    sess = tf.Session(config=util.get_config())
...
def run_eval(model, batcher, vocab):
    """Repeatedly runs eval iterations, logging to screen and writing summaries. Saves the models with the best loss seen so far."""
    model.build_graph()  # build the graph
    ...
    while True:
        ...
        if best_loss is None or running_avg_loss < best_loss:
            tf.logging.info('Found new best models with %.3f running_avg_loss. Saving to %s', running_avg_loss,
                            bestmodel_save_path)
...
def main(unused_argv):
    ...
    if hps.mode == 'train':
        ...
        decode_model_hps = hps  # This will be the hyperparameters for the decoder models
        decode_model_hps = hps._replace(
            max_dec_steps=1)  # The models is configured with max_dec_steps=1 because we only ever run one step of the decoder at a time (to do beam search). Note that the batcher is initialized with max_dec_steps equal to e.g. 100 because the batches need to contain the full summaries
